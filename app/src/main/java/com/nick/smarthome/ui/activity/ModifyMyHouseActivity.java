...
import com.nick.smarthome.callback.MyHouseInfoCallback;
 ...
 * Description:编辑我的房源
 */
public class ModifyMyHouseActivity extends BaseSwipeBackActivity implements View.OnClickListener,
        AMapLocationListener {

    private static final String TAG = "ModifyMyHouseActivity";
    ...
    private String mCustomerId, houseId;
    ...
    private void initDatas() {
        ...
        String url = ServerApiConstants.Urls.GET_MY_HOUSE_INFO_URLS;
        ...
        OkHttpUtils
                ...
                .build()
                .execute(new MyHouseInfoCallback() {
                    @Override
                    ...
                    @Override
                    public void onResponse(MyHouseInfoResult response) {

                        if (response.getStatuscode().equals("1")) {
                            MyHouseInfoResult.DataEntity.HouseInfoEntity houseInfo = response.getData().getHouseInfo();
                            tvTitle.setText(houseInfo.getHouseTitle());
                            tvAddr.setText(houseInfo.getAddress());
                            latitude = String.valueOf(houseInfo.getLatitude());
                            longitude = String.valueOf(houseInfo.getLongitude());

                            ArrayList<MyHouseInfoResult.DataEntity.RnLockListEntity> lockList = (ArrayList<MyHouseInfoResult.DataEntity.RnLockListEntity>) response.getData().getRnLockList();
                            int len = lockList.size();

                            for (int i = 0; i < len; i++) {
                                MyHouseInfoResult.DataEntity.RnLockListEntity lockEntity = lockList.get(i);
                                MyLockEntity selectLock = new MyLockEntity();
                                selectLock.setLockCode(lockEntity.getLockCode());
                                selectLock.setRnLockId(lockEntity.getRnLockId());
                                selectLock.setLockType(lockEntity.getLocktype());
                                selectedLockList.add(selectLock);
                            }
    ...
    private void submitHouseInfo() {
        ...
        String url = ServerApiConstants.Urls.UPDATE_MY_HOUSE_INFO_URLS;
        ...
        try {
            ...
            rnHouseInfoJson.put("houseId", houseId);
        ...
        params.put("houseInfoJson", rnHouseInfoJson.toString());
        params.put("rnLockList", lockIdJsonStr);
